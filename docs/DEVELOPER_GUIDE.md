...
mods' members. Minecraft 26.3 retains live named holder sets rather than a
...
All Mineralogy recipes are native Minecraft/NeoForge 26.3 JSON under
...
NeoForge 26.3 converts pre-flattening chunks lazily. Required Mixins expand
...
Minecraft 26.3's four-argument chunk upgrader is intercepted before and after
...
Minecraft 26.3 renamed the modern block-state NBT fields to lowercase `id`
and `properties`. The pre-flattening `BlockStateData` data-fixer input remains
the historical uppercase `Name`/`Properties` schema, so Mineralogy serializes
that legacy compound explicitly instead of calling the current
`NbtUtils.writeBlockState`. The target data fixer must then be allowed to
produce the modern lowercase representation.
...
[Mineralogy Versioning](VERSIONS.md). This branch validates target `2603002`
for Minecraft 26.3 NeoForge and does not append CI build numbers.
