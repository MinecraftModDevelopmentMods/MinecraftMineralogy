...
Mineralogy 6 needs Minecraft 1.12.2, Forge 14.23.5.2859, and OreSpawn
4.0.7.112021.
...
Minecraft 1.12 accepts Y `0` through `255`; Depth Spread accepts `1` through
...
The terrain-host list is not exposed by OreSpawn's 1.12 graphical editor. Stop
...
Minecraft 1.12 these resolve through OreSpawn's compatibility mapping and Ore
Dictionary names. Exact `host_blocks` entries are the clearest choice for one
modded stone.

Minecraft 1.12 matches a `host_blocks` entry by block registry identity, not by
...
separate Mineralogy dimension option. Minecraft 1.12 identifies a custom
...
The 1.12 graphical editor does not expose terrain-dimension or rock-membership
