...
Mineralogy 6 needs Minecraft 1.19.4, Forge 45.4.0, and OreSpawn
4.0.16.119041.
...
On the first 1.19.4 start, Mineralogy recognizes the old saved registry and
...
Minecraft 1.19.4 accepts Y `-64` through `319`; Depth Spread accepts `1` through
...
The terrain-host list is not exposed by OreSpawn's 1.19.4 graphical editor. Stop
...
Minecraft 1.19.4 these resolve through target-native block tags. Exact
`host_blocks` entries are the clearest choice for one
modded stone.

Minecraft 1.19.4 matches a `host_blocks` entry by flattened block registry
identity. Add only
...
separate Mineralogy dimension option. Minecraft 1.19.4 uses the dimension
type's registered ID. For example, use `examplemod:moon` when that is the ID
documented by the installed dimension mod.

The 1.19.4 graphical editor does not expose terrain-dimension or rock-membership
