...
On the first 26.3 start, Mineralogy recognizes the old saved registry and
...
Minecraft 26.3 stores new Overworld region files below
...
Minecraft 26.3 also has its own sulfur and cinnabar families. They remain
...
Minecraft 26.3 accepts Y `-64` through `319`; Depth Spread accepts `1` through
...
The terrain-host list is not exposed by OreSpawn's 26.3 graphical editor. Stop
...
Minecraft 26.3 these resolve through target-native block tags. Exact
`host_blocks` entries are the clearest choice for one
modded stone.

Minecraft 26.3 matches a `host_blocks` entry by flattened block registry
identity. Add only
...
separate Mineralogy dimension option. Minecraft 26.3 uses the dimension
type's registered ID. For example, use `examplemod:moon` when that is the ID
documented by the installed dimension mod.

The 26.3 graphical editor does not expose terrain-dimension or rock-membership
