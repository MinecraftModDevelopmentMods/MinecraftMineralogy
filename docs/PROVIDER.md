...
- Minecraft 26.3 sulfur and cinnabar remain separate native families; they are
  not aliases, recipe inputs, tags, or outputs of Mineralogy's sulfur rule.
- Minecraft 26.3 heights `-64` through `319` and Overworld-only terrain defaults.
...
Minecraft 26.3 uses the registered dimension-type ID, such as
...
The 26.3 editor does not expose terrain-dimension or rock-membership fields, so
...
`minecraft:deepslate` hosts. Forge 66 binds tags after common setup, while
...
On Minecraft 26.3, terrain hosts are baked as flattened block identities.
