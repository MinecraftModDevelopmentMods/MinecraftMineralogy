...
- Minecraft 1.14.4 heights `0` through `255` and Overworld-only terrain defaults.
- Flattened Minecraft blocks for granite, diorite, and andesite; Mineralogy
  blocks for basalt and tuff because Minecraft 1.14.4 does not supply them.
...
Minecraft 1.14.4 uses the registered dimension-type ID, such as
...
The 1.14.4 editor does not expose terrain-dimension or rock-membership fields, so
...
On Minecraft 1.14.4, terrain hosts are baked as flattened block identities.
