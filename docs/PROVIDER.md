...
- Minecraft 1.15.2 heights `0` through `255` and Overworld-only terrain defaults.
- Flattened Minecraft blocks for granite, diorite, and andesite; Mineralogy
  blocks for basalt and tuff because Minecraft 1.15.2 does not supply them.
...
Minecraft 1.15.2 uses the registered dimension-type ID, such as
...
The 1.15.2 editor does not expose terrain-dimension or rock-membership fields, so
...
On Minecraft 1.15.2, terrain hosts are baked as flattened block identities.
