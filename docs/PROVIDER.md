...
- Minecraft 1.13 heights `0` through `255` and Overworld-only terrain defaults.
- Flattened Minecraft blocks for granite, diorite, and andesite; Mineralogy
  blocks for basalt and tuff because Minecraft 1.13 does not supply them.
...
Minecraft 1.13 maps a custom numeric dimension `N` to
...
The 1.13 editor does not expose terrain-dimension or rock-membership fields, so
...
On Minecraft 1.13, terrain hosts are baked as flattened block identities.
