...
this Minecraft 1.12.2 Forge release is therefore:
...
The 1.12.2 row records this branch's current release. The other rows illustrate
...
```text
6.0.1.112021 -> 6.1.0.112021
6.1.4.112021 -> 7.0.0.112021
```
...
Mineralogy 6 also requires OreSpawn `[4.0.7,5.0.0)`. OreSpawn uses the same
target calculation, so the matching Minecraft 1.12.2 Forge release is
`4.0.7.112021`. The dependency range deliberately describes the supported
...
branch, published metadata and artifacts therefore use `6.0.1.112021`.
