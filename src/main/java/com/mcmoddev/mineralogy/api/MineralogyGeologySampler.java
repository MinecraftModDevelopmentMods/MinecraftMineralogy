...
import com.mcmoddev.mineralogy.worldgen.BakedGeomeConfig;
import com.mcmoddev.mineralogy.worldgen.Geology;
import com.mcmoddev.mineralogy.worldgen.GeomeConfig;
import com.mcmoddev.mineralogy.worldgen.GeomeGeology;
...
final class MineralogyGeologySampler implements GeologySampler {
	private static final ResourceLocation CYANO_GEOME = new ResourceLocation("mineralogy", "cyano");
	...
	private MineralogyGeologySampler(ServerLevel level) {
		this.level = level;
	...
	static GeologySampler create(ServerLevel level) {
		if (level == null || WorldGeologyProfileManager.activeServer() != level.getServer()) {
			throw new IllegalStateException("The level is not part of Mineralogy's active server");
		}
		return new MineralogyGeologySampler(level);
	}
	...
	public GeologyColumn sampleColumn(int blockX, int blockZ, int surfaceY) {
		...
		ResourceLocation biomeId = holder.unwrapKey().map(ResourceKey::location)
				.orElse(new ResourceLocation("mineralogy", "unregistered_biome"));
		if (mode == GeologyMode.LEGACY) {
	...
	private static ResourceLocation geomeId(String name) {
		try {
			return name.indexOf(':') >= 0 ? new ResourceLocation(name)
					: new ResourceLocation("mineralogy", name.toLowerCase(Locale.ROOT));
		} catch (RuntimeException ignored) {
			return new ResourceLocation("mineralogy", "unknown");
		}
	}
}
