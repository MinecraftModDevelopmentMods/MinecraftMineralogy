...
final class BlockAssignmentScreen extends Screen {
	...
	BlockAssignmentScreen(Screen parent, GeologyEditorSession session, String blockId) {
		super(new TranslatableComponent("screen.mineralogy.assign_block"));
		this.parent = parent;
	...
	protected void init() {
		...
		int right = width / 2 + 5;
		addRenderableWidget(new Button(left, 70, 150, 20, new TranslatableComponent("tab.mineralogy.sedimentary"),
				button -> assignRock(RockFamily.SEDIMENTARY)));
		addRenderableWidget(new Button(right, 70, 150, 20, new TranslatableComponent("tab.mineralogy.metamorphic"),
				button -> assignRock(RockFamily.METAMORPHIC)));
		addRenderableWidget(new Button(left, 98, 150, 20, new TranslatableComponent("value.mineralogy.intrusive"),
				button -> assignRock(RockFamily.IGNEOUS_INTRUSIVE)));
		addRenderableWidget(new Button(right, 98, 150, 20, new TranslatableComponent("value.mineralogy.volcanic"),
				button -> assignRock(RockFamily.IGNEOUS_VOLCANIC)));
		addRenderableWidget(new Button(left, 126, 310, 20, new TranslatableComponent("tab.mineralogy.ores"),
				button -> assignOre()));
