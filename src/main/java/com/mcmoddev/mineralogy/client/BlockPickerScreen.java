...
import com.mcmoddev.mineralogy.client.GeologyEditorSession.MaterialTab;
...
final class BlockPickerScreen extends Screen {
	...
	BlockPickerScreen(GeologyMaterialsScreen parent, GeologyEditorSession session, MaterialTab target) {
		super(new TranslatableComponent("screen.mineralogy.choose_block"));
		this.parent = parent;
	...
	protected void init() {
		...
		search = addRenderableWidget(new EditBox(font, left, 34, 230, 20,
				new TranslatableComponent("option.mineralogy.search")));
		search.setMaxLength(128);
		search.setValue(searchText);
		addRenderableWidget(new Button(left + 235, 34, 75, 20,
				new TranslatableComponent("button.mineralogy.search"), button -> {
		...
		addRenderableWidget(CycleButton.builder(this::namespaceName)
				...
				.create(left, 58, 150, 20, new TranslatableComponent("option.mineralogy.mod_filter"),
						(button, value) -> { namespace = value; page = 0; rebuildWidgets(); }));
		addRenderableWidget(new Button(left + 160, 58, 150, 20,
				new TranslatableComponent(showAll ? "button.mineralogy.safe_only" : "button.mineralogy.show_all"),
	...
	private Component namespaceName(String value) {
		return value.isEmpty() ? new TranslatableComponent("value.mineralogy.all_mods") : new TextComponent(value);
	}
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		drawCenteredString(poseStack, font,
				new TranslatableComponent("label.mineralogy.adding_to",
						new TranslatableComponent("tab.mineralogy." + target.key)),
				width / 2, 20, 0xCCCCCC);
