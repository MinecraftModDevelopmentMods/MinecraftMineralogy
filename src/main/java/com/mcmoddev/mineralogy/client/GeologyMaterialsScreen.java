...
final class GeologyMaterialsScreen extends Screen {
	...
	GeologyMaterialsScreen(Screen parent, GeologyEditorSession session) {
		super(new TranslatableComponent("screen.mineralogy.materials"));
		this.parent = parent;
	...
	protected void init() {
		...
		for (int i = 0; i < MaterialTab.values().length; i++) {
			...
			Button button = addRenderableWidget(new Button(tabX, 26 + (tabRow * 24), tabWidth, 20,
					new TranslatableComponent("tab.mineralogy." + value.key), selected -> changeTab(value)));
		...
		for (int i = 0; i < pageSize && start + i < ids.size(); i++) {
			...
			if (tab != MaterialTab.UNASSIGNED) {
				addRenderableWidget(new Button(contentLeft + entryWidth + rowGap, y, removeWidth, 20,
						new TranslatableComponent("button.mineralogy.remove"), button -> removeEntry(id)));
			}
		}
		...
		Button add = addRenderableWidget(new Button(contentLeft + contentWidth - addWidth, controlsY, addWidth, 20,
				new TranslatableComponent("button.mineralogy.add_block"), button -> openBlockPicker()));
	...
	private String rowLabel(String id) {
		if (tab == MaterialTab.ORES) {
			...
			return source.isEmpty() ? id : id + "  [" + source + "]";
		}
		return id;
	}
