...
final class GeomeEntryScreen extends Screen {
	...
	GeomeEntryScreen(Screen parent, GeologyEditorSession session, String geomeId) {
		super(new TranslatableComponent("screen.mineralogy.geome_entry"));
		this.parent = parent;
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		drawString(poseStack, font, new TranslatableComponent("option.mineralogy.base_weight"),
				width / 2 - 155, 58, 0xDDDDDD);
		int index = 0;
		for (RockFamily family : RockFamily.values()) {
			drawString(poseStack, font, new TranslatableComponent("value.mineralogy.family." + family.configName),
