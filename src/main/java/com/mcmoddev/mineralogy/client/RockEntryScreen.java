...
final class RockEntryScreen extends Screen {
	...
	RockEntryScreen(Screen parent, GeologyEditorSession session, String blockId) {
		super(new TranslatableComponent("screen.mineralogy.rock_entry"));
		this.parent = parent;
	...
	protected void init() {
		...
		addRenderableWidget(CycleButton.builder(this::familyName)
				...
				.create(left, 38, 190, 20, new TranslatableComponent("option.mineralogy.family"),
						(button, value) -> family = value));
		addRenderableWidget(CycleButton.onOffBuilder(enabled)
				.create(left + 200, 38, 110, 20, new TranslatableComponent("option.mineralogy.enabled"),
		...
		addRenderableWidget(CycleButton.onOffBuilder(oreReplaceable)
				.create(left, 176, 150, 20, new TranslatableComponent("option.mineralogy.ore_replaceable"),
		...
		addRenderableWidget(new Button(right + 45, bottom, 105, 20,
				new TranslatableComponent("button.mineralogy.remove"), button -> remove()));
	}
	...
	private Component familyName(RockFamily value) {
		return new TranslatableComponent("value.mineralogy.family." + value.configName);
	}
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		drawCenteredString(poseStack, font, new TextComponent(blockId), width / 2, 20, 0xDDDDDD);
