...
public final class LegacyWorldDataHook {
	...
	public static void registerWorldPersistenceHook() {
		WorldPersistenceHooks.addHook(new WorldPersistenceHooks.WorldPersistenceHook() {
			...
			@Override
			public void readData(SaveFormat.LevelSave levelSave, IServerConfiguration serverInfo,
					CompoundNBT fmlData) {
	...
	public static void onServerAboutToStart(FMLServerAboutToStartEvent event) {
		File levelDat = event.getServer().func_240776_a_(FolderName.LEVEL_DAT).toFile();
	...
	private static synchronized void prepareLegacyWorld(File worldDirectory, CompoundNBT fmlData) {
		legacyWorldActive = false;
		...
		if (sidecar.isFile()) {
			try (FileInputStream input = new FileInputStream(sidecar)) {
				install(worldDirectory, CompressedStreamTools.readCompressed(input).getCompound("Blocks"));
			} catch (IOException e) {
				LOGGER.warn("Could not read legacy Mineralogy registry sidecar '{}'", sidecar, e);
			}
		}
