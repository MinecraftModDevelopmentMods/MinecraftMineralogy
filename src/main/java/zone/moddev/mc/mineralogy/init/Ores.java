...
public class Ores {
    ...
    private static Block addOre(String oreDictionaryName, Item oreDropItem, int numMin, int numMax, int pickLevel) {
        ...
        addBlock(oreDictionaryName);

        return oreBlock;
    }

    private static Block addBlock(String oreDictionaryName) {
        String name = oreDictionaryName.toLowerCase() + "_block";

        return RegistrationHelper.registerBlock(
                new Rock(false, (float) 1.5, (float) 10, 0, SoundType.STONE),
                name, Constants.BLOCK.toLowerCase() + oreDictionaryName).PairedBlock;
    }
}
