package zone.moddev.mc.mineralogy.ioc;
...
import zone.moddev.mc.mineralogy.lib.interfaces.IDynamicTabProvider.DefaultTabGenerationMode;
import zone.moddev.mc.mineralogy.lib.util.DynamicTabProvider;
...
public class MinIoC {
    ...
    public void wireup() {
        ...
        DynamicTabProvider provider = new DynamicTabProvider();
        if (MineralogyConfig.groupCreativeTabItemsByType()) {
            provider.addTab("Rock", true, Mineralogy.MODID)
            ...
            provider.setDefaultTabCreationLogic(DefaultTabGenerationMode.ByMod)
                    .addTab(Mineralogy.MODID, true, Mineralogy.MODID);
        }
        this.register(IDynamicTabProvider.class, provider);
