...
public class TileEntityRockFurnace extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    private boolean recoverBurnModifier;

    /** Required by Forge when a saved tile entity is reconstructed. */
    public TileEntityRockFurnace() {
        _burnModifier = 1.0F;
        recoverBurnModifier = true;
    }

    public TileEntityRockFurnace(float burnModifier) {
            _burnModifier = burnModifier;
            recoverBurnModifier = false;
    }
...
//    }
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityRockFurnace.class, new String[] {"Items"}));
    }
    ...
    private void recoverBurnModifier() {
        if (!recoverBurnModifier || this.world == null || this.pos == null) {
            return;
        }
        Block block = this.world.getBlockState(this.pos).getBlock();
        _burnModifier = burnModifierFor(block, _burnModifier);
        recoverBurnModifier = false;
    }

    static float burnModifierFor(Block block, float fallback) {
        return block instanceof RockFurnace ? ((RockFurnace) block).getBurnModifier() : fallback;
    }
