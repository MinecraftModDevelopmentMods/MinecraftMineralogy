...
public class NamespaceAndLocaleContractTest {
    ...
    public void namespaceVersionAndOilIdentitiesAreStable() {
        ...
        assertEquals("6.0.1.112021", Mineralogy.VERSION);
    ...
    public void metadataRequiresReleasedOreSpawnRange() throws Exception {
        ...
        assertEquals("orespawn@[4.0.7,5.0.0)", mod.getAsJsonArray("dependencies").get(0).getAsString());
    }
    ...
    public void allSeventeenLocalesHaveExactOrderedKeyParity() throws Exception {
        ...
        assertEquals(938, expected.size());
        for (File file : files) {
    ...
    public void retiredProductionPackagesAndWorldgenAreAbsent() throws Exception {
        ...
        assertTrue("legacy saprolite and pummice aliases must be initialized before registry events",
                joined.contains("PatchHandler.getInstance().init(MineralogyConfig.patchUpdate());"));
    }
